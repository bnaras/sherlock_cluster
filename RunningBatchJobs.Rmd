---
title: "Running Batch Jobs on Sherlock and Sherlock2"
...
3. Use the _job_ template below.  Edit it suitably and save in
   `job_template.sh`. _Note that `sherlock` and `sherlock2` differ in
...
5. If you use Gurobi or any optimization package, you need to install
the approproiate R interface. Figure out the version of Gurobi you
want using `module avail` at the shell prompt. Then load it, after
which you can install the R package.

```{bash, eval = FALSE}
[login_node ~]$ module load gurobi/6.0.4
[login_node ~] R CMD INSTALL ${GUROBI_HOME}/R/*.gz
```

Note that use of the Gurobi package in R requires the `slam`
package. However, the current version of the `slam` package on CRAN is
is designed to work with R version 3.3.1 and above. Since sherlock
only has R version 3.3.0 installed at the moment, one kludge is to
install the older version
[slam](https://cran.r-project.org/src/contrib/Archive/slam/slam_0.1-37.tar.gz)
manually.
